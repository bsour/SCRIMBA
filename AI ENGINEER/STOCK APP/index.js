...
async function fetchStockData() {
  ...
  try {
    const stockData = await Promise.all(
      tickersArr.map(async (ticker) => {
        ...
        if (response.ok) {
          ...
          // Process data (Remove unnecessary properties)
    ...
    // Ensure err.message exists before logging
    if (err.message) {
      console.error("Error Message:", err.message);
    } else {
      console.error("Error object does not have a 'message' property.");
    }
  }
}
...
async function fetchReport(data) {
  ...
  try {
    const url = "https://open-ai-api.sourabhbeniwal.workers.dev";
